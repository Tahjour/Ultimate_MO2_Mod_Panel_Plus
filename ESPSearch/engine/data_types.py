...
if TYPE_CHECKING:
    ...
    from .load_order import LoadOrderInfo
...
class RecordConflict:
    """Represents conflict information for a record across multiple plugins."""
...
class ConflictAnalysisResult:
    """Result of a conflict analysis for a set of records."""
...
# Record flags common across Bethesda games
...
@dataclass
class Record:
    """A record in a plugin file (e.g., ARMO, WEAP, etc.)."""
    signature: str
    data_size: int
    flags: int
    form_id: int
    ...
    def get_content_hash(self) -> str:
        """Returns a hash of the record content for fast comparison."""
        if self._content_hash is None:
            hasher = hashlib.md5()
            # Hash metadata that affects structure
            hasher.update(self.signature.encode('ascii'))
            hasher.update(self.flags.to_bytes(4, 'little'))
            # Hash all fields (signature + data)
    ...
    def flag_names(self) -> List[str]:
        """Returns a list of human-readable flag names."""
        names: List[str] = []
...
@dataclass
class Group:
    """A group of records or other groups in a plugin file."""
    label_raw: bytes
    group_type: int
    group_size: int
    stamp: int
...
class LazyPluginIndex:
    """A lightweight index of record offsets in a plugin file."""
    path: str
    header_size: int
...
@dataclass
class PluginFile:
    """A parsed ESP/ESM/ESL plugin file."""
    path: str
    file_size: int
    header_size: int
    localized: bool
