...
class StructureNode:
    """Внутренний узел для StructureItemModel."""
    def __init__(self, key: any, name: str, parent: Optional['StructureNode'] = None) -> None:
        self.key = key
        self.name = name
        self.parent = parent
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None) -> None:
        super().__init__(parent)
        self.setWindowTitle("ESP/ESM/ESL Viewer")
    ...
    def _setup_actions(self) -> None:
        ...
        self.addAction(self.action_expand_all)
        self.addAction(self.action_collapse_all)
        self.addAction(self.action_export)
        self.addAction(self.action_export_all)
        self.addAction(self.action_adv_search)
        self.addAction(self.action_quick_filter)
    ...
    def _setup_toolbar(self) -> None:
        ...
        # Search-строка в самом левом краю панели
        ...
        self._conflict_only_box = QCheckBox("Show only conflicts", self)
        self._conflict_hide_identical_box = QCheckBox("Hide identical to master", self)
        self._conflict_mode_combo = QComboBox(self)
        self._conflict_mode_combo.addItem("All conflicts", "all")
        self._conflict_mode_combo.addItem("Winning conflicts", "winning")
        self._conflict_mode_combo.addItem("Losing conflicts", "losing")
        ...
        self._conflict_only_box.toggled.connect(
            lambda checked: self._on_conflict_filter_changed(
                checked,
                bool(self._conflict_hide_identical_box and self._conflict_hide_identical_box.isChecked()),
            )
        )
        self._conflict_hide_identical_box.toggled.connect(
            lambda checked: self._on_conflict_filter_changed(
                bool(self._conflict_only_box and self._conflict_only_box.isChecked()),
                checked,
            )
        )
        ...
        self.tools_toolbar = QToolBar("Search", self)
        ...
        self.tools_button.setAccessibleName("Search flyout")
        self.tools_button.setAccessibleDescription("Open advanced search panel")
        self.tools_button.clicked.connect(lambda: self._toggle_flyout(self.search_flyout, self.tools_button))
        self.tools_toolbar.addWidget(self.tools_button)
        self.tools_toolbar.setToolTip("Search")
    ...
    def _setup_menu(self) -> None:
        ...
        save_menu.addAction(self.action_export_all)
    ...
    def _setup_tools_panel(self) -> None:
        ...
        self.tools_panel.setAccessibleName("Advanced search panel")
        self.tools_panel.setAccessibleDescription("Advanced search and results")
    ...
    def _setup_layout(self) -> None:
        ...
        conflict_header = self.conflict_view.header()
        conflict_header.setStretchLastSection(False)
        conflict_header.setSectionResizeMode(0, QHeaderView.ResizeMode.Interactive)
        conflict_header.setSectionResizeMode(1, QHeaderView.ResizeMode.Interactive)
        conflict_header.setSectionResizeMode(2, QHeaderView.ResizeMode.Interactive)

        structure_header = self.structure_view.header()
        structure_header.setStretchLastSection(False)
        structure_header.setSectionResizeMode(0, QHeaderView.ResizeMode.Interactive)
        ...
        max_height = max(
            self.left_filter_box.sizeHint().height(),
            self.right_filter_box.sizeHint().height(),
        ) + 12
        filter_panel.setMaximumHeight(max_height)
        ...
        central_layout.addWidget(filter_panel)
    ...
    def _apply_default_column_sizes(self) -> None:
        ...
        base_width = max(280, tree_header.sectionSizeHint(0))
        self.tree_view.setColumnWidth(0, base_width)
        ...
        detail_width = max(460, detail_header.sectionSizeHint(1))
        self.detail_view.setColumnWidth(1, detail_width)

        # Добавляем размеры для таблицы структуры
        self.structure_view.resizeColumnToContents(0)
        struct_header = self.structure_view.header()
        struct_width = max(300, struct_header.sectionSizeHint(0))
        self.structure_view.setColumnWidth(0, struct_width)
    ...
    def _restore_ui_state(self) -> None:
        ...
        search_state = self._settings.value("search_header", type=QByteArray)
        if search_state and not search_state.isEmpty():
            self.tools_panel.restore_search_header_state(search_state)

        bottom_splitter_state = self._settings.value("bottom_splitter", type=QByteArray)
        if bottom_splitter_state and not bottom_splitter_state.isEmpty():
            self.bottom_splitter.restoreState(bottom_splitter_state)

        detail_index = self._settings.value("detail_tab_index")
        if detail_index is not None:
            try:
                index_int = int(detail_index)
            except (TypeError, ValueError):
                index_int = -1
            if 0 <= index_int < self.detail_tabs.count():
                self.detail_tabs.setCurrentIndex(index_int)

        only_conflicts = self._settings.value("conflict_only")
        hide_identical = self._settings.value("conflict_hide_identical")
        mode_value = self._settings.value("conflict_mode")
        if isinstance(only_conflicts, bool):
            self._conflict_filter_only = only_conflicts
        if isinstance(hide_identical, bool):
            self._conflict_filter_hide_identical = hide_identical
        if isinstance(mode_value, str):
            self._conflict_filter_mode = mode_value
            if self._conflict_mode_combo is not None:
                for i in range(self._conflict_mode_combo.count()):
                    data = self._conflict_mode_combo.itemData(i)
                    if data == mode_value:
                        self._conflict_mode_combo.blockSignals(True)
                        self._conflict_mode_combo.setCurrentIndex(i)
                        self._conflict_mode_combo.blockSignals(False)
                        break
        self._sync_conflict_controls(self._conflict_filter_only, self._conflict_filter_hide_identical)

        has_tree_state = self._settings.contains("tree_header")
        has_detail_state = self._settings.contains("detail_header")
        if not has_tree_state and not has_detail_state:
    ...
    def _save_ui_state(self) -> None:
        ...
        self._settings.setValue("bottom_splitter", self.bottom_splitter.saveState())
        
        # Save header states and explicit widths
        for key, view in [
            ("tree_header", self.tree_view),
            ("detail_header", self.detail_view),
            ("structure_header", self.structure_view),
            ...
            self._settings.setValue(f"{key}_widths", widths)

        self._settings.setValue("search_header", self.tools_panel.save_search_header_state())
        self._settings.setValue("detail_tab_index", self.detail_tabs.currentIndex())
        self._settings.setValue("conflict_only", self._conflict_filter_only)
        self._settings.setValue("conflict_hide_identical", self._conflict_filter_hide_identical)
        self._settings.setValue("conflict_mode", self._conflict_filter_mode)
        self._settings.endGroup()
    ...
    def _on_conflict_filter_changed(self, only_conflicts: bool, hide_identical: bool) -> None:
        self._conflict_filter_only = only_conflicts
        self._conflict_filter_hide_identical = hide_identical
        self._sync_conflict_controls(only_conflicts, hide_identical)
        self._apply_filters()
        if self._conflict_filter_only:
            self.tree_view.expandAll()

    def _on_conflict_mode_changed(self, index: int) -> None:
        if not self._conflict_mode_combo:
            return
    ...
    def _resolve_full_name(self, record: Record) -> Optional[str]:
        if record.full_name:
            return record.full_name
        if self.current_plugin is None:
            return None
        return extract_string_field(record.fields, ("FULL", "DESC"), self.current_plugin)
